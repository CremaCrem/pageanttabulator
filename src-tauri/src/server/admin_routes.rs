...
pub async fn compute_results(
    ...
    match payload.round.as_str() {
        "preliminary" => {
            ...
            for score in scores {
                score_map.entry((score.candidate_id, score.segment_id))
            ...
            let segments = vec!["production_number", "school_uniform", "professional_attire", "modern_barong"];
        ...
        "final" => {
            ...
            for res in results.iter_mut() {
                ...
                // Get gender
                let gender = candidates.iter().find(|c| c.id == res.candidate_id).map(|c| c.gender.as_str()).unwrap_or("");
                
                // Final QA avg
                let mut final_qa_avg = 0.0;
                if let Some(list) = score_map.get(&(res.candidate_id.clone(), "final_qa".to_string())) {
                    if !list.is_empty() {
                        let sum: f64 = list.iter().sum();
                ...
                res.final_score = Some(crate::scoring::compute::compute_final_score(prelim, final_qa_avg));
...
pub async fn get_special_awards(
    State(state): State<AppState>,
) -> Json<Value> {
    let conn = state.db.lock().unwrap();
