...
pub async fn claim_session(
    ...
    let token = payload.device_token.unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
...
pub async fn reset_session(
    ...
    if let Ok(_) = conn.execute("UPDATE judges SET is_active = 0, session_token = NULL WHERE id = ?1", rusqlite::params![judge_id]) {
